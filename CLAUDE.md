...
- **Known gaps (NOT yet closed — flag before relying on them):** (1) **roles are still global** — an Admin/Editor is one in every site; per-site role membership + a cross-site super-admin is deferred (Phase 5). Section *scopes* are already per-site. (2) **the MCP is pinned to the Default site** — `getAccessContext` is built without an active site, so agents cannot manage another site until a `PAPERBOY_SITE`-style selector exists. (Closed 2026-08-31: reference and content-area `ref` targets are now resolved at write — a missing, trashed, cross-site, wrong-kind or wrong-type target is refused with a self-teaching 422; `reference-target-validation.test.ts`.)
