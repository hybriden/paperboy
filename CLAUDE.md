...
- `packages/preview` — `@paperboycms/preview` (published to npm): the framework-agnostic on-page-editing bridge for the preview iframe, zero runtime dependencies. Single source of truth for the admin↔frontend postMessage protocol (`paperboy:edit/drop/add-block/rect/patch/focus`) and the `data-pb-*` DOM contract; the admin and `apps/web` both import it — never re-declare message shapes elsewhere.
...
- MCP **8093** (optional, opt-in): `MCP_TOKEN=mcp_… docker compose --profile mcp up -d --no-deps mcp` serves the MCP over Streamable HTTP at `/mcp` (Bearer = `MCP_TOKEN`). Default is stdio; HTTP mode is only for remote clients. **`PAPERBOY_SITE`** (site slug or id) selects the site the server acts in — unset means the Default site, and an unknown value refuses to boot.
...
## Auth model
- Browser: argon2id + opaque server-side session cookie (`__Host-paperboy_sid` when `COOKIE_SECURE=true`) + CSRF (a synchronizer token bound to the session row, echoed in `X-CSRF-Token`, PLUS a fail-closed Origin/Referer check — there is no CSRF cookie; `apps/api/src/security.ts`) + rate-limit/lockout.
...
- **MCP tokens are SITE-SCOPED** (migration 0028, `mcp_token.site_id`) — minted against the active site like a delivery key, which is the precedent (`verifyMcpToken` now returns `{userId, siteId}`, shaped like `verifyDeliveryKey`). The scope is a CREDENTIAL-level cap: it sets the server's default site and bounds every per-call `site`, so a scoped token asking for another site is refused, not quietly served. `NULL` = every site — what existing rows get (nothing in flight changes on deploy) and what Settings → MCP's "Every site" checkbox mints for a cross-site agent. The cap travels in a PER-REQUEST scope (`AsyncLocalStorage`, `apps/mcp/src/server.ts`), not a module variable, so one HTTP server serves a site-scoped and a cross-site token at once and neither inherits the other's reach — pinned by `mcp-http-transport.test.ts`.
- **An MCP token is minted as the SIGNED-IN user** — Settings → MCP asks only for a name. It inherits that user's roles and section scopes, so an agent can never exceed whoever created it; to give an agent less, sign in as the narrower account and mint it there. There is no password re-auth (`POST /users` can already create a fresh Admin without one, so the gate only closed one of two doors). `POST /manage/mcp-tokens` still accepts an optional `userId` — `user.manage` only, not offered in the UI — to provision a token for a service account.
- **MCP token revocation takes effect immediately.** The presented bearer is resolved through the DB *before* any constant-time compare against the in-memory boot `MCP_TOKEN`, so a token revoked in Settings → MCP stops working without a restart; `AccessContext` is also re-resolved per HTTP request, so role/scope changes apply live. **HTTP mode only:** a running stdio process authenticates once at boot and keeps that identity until it exits. An env-only `MCP_TOKEN` with no DB row still works (rotate it by restarting).
...
- **Known gaps (NOT yet closed — flag before relying on them):** (1) **roles are still global** — an Admin/Editor is one in every site; per-site role membership + a cross-site super-admin is deferred (Phase 5). Section *scopes* are already per-site. (2) ~~the MCP is pinned to the Default site~~ — **CLOSED 2026-09-09**: `PAPERBOY_SITE` (slug or id) pins an MCP server to a site, resolved at boot and threaded into `getAccessContext` on BOTH the stdio and HTTP paths; `list_sites` reports what exists and which one is active; an unknown value REFUSES to boot, naming the sites that exist, rather than silently serving the Default. **One server reaches EVERY site**: `PAPERBOY_SITE` only sets the default, and every tool takes an optional `site` (slug or id) that overrides it per call — no second process per site. The per-call context is AsyncLocalStorage-backed (`callCtx`, `ctx()` in apps/mcp) precisely so two OVERLAPPING calls naming different sites can't bleed; a plain mutable module-level `ctx` would let one answer for the wrong site. An unknown `site` argument is a self-teaching ERROR, never empty results — an empty list would read as "this site has no pages", which is the original bug wearing a different hat. Found live (two sites; every tool returned the Default's content and `delivery_search` for the other site's only page returned `{"items":[],"total":0}`) — note the REST API had always threaded `x-paperboy-site` and delivery keys carry their own `site_id`, so the MCP was the one surface with no door. Pinned by `mcp-site-selection.test.ts`. (Closed 2026-08-31: reference and content-area `ref` targets are now resolved at write — a missing, trashed, cross-site, wrong-kind or wrong-type target is refused with a self-teaching 422; `reference-target-validation.test.ts`.)
- **Deploy:** `0012`/`0013` are additive/idempotent and run on api boot like any migration — no reseed. Status: **merged to main** (PR #2, plus per-site setup/UX in PRs #3–#8); the two known gaps above are still open.
...
3. **All tolerant coercion lives in ONE chokepoint** — `coerceFieldValue` (packages/shared), applied server-side in `updateContent` so the API, the MCP, the agent and the admin all pass through it (the admin never coerces locally), test-pinned in `update-ergonomics.test.ts`. Mistakes it absorbs (each from a real run): self-keyed wrap `{field: v}`, locale-map wrap `{en: v}`, TipTap doc → real Markdown (structure kept) / separated plain text, string → TipTap doc, single block → array, resolved asset object → documentId, richtext outside the editor schema → normalized. Add new agent mistakes HERE, with a test.
   **The chokepoint reaches the WHOLE document, not just its top level.** `coerceData` takes a `BlockTypeResolver` and recurses into content-area `inline` block data (depth-capped), because it previously stopped at the first level: a block's `richtext` field kept raw Markdown — persisted, 200 OK, rendered blank — while the identical top-level field was correctly parsed. Pinned by `shared-coerce-blocks.test.ts` + `inline-block-coercion.test.ts`. Strict *validation* of `inline` against the block's own schema and rejection of an unknown `blockType` are implemented too, at every nesting level (`contentAreaSchemaFor` in packages/shared + `assertPlacement` in packages/db), pinned by `unknown-blocktype.test.ts`. **Still open:** TOP-LEVEL unknown keys pass through (`dataSchemaFor` is `passthrough()` at the document root, pinned by `shared-schema-hints.test.ts`) — `data:{titel:"x"}` on a page persists with 200 and is never delivered.
...
6. **Every failed agent run must leave a trail.** MCP tool errors log to stderr WITH truncated args (stdout is the JSON-RPC channel in stdio mode; `docker logs` shows both); every MCP write audit-logs like the API routes (`ip='mcp'`). Two incidents were undiagnosable because errors only travelled in-band and the client swallowed them.
...
- **Coverage is measured AND enforced.** `pnpm test:coverage` runs the API suite with v8
  ...
  `apps/api/vitest.config.ts` gate the build. Baseline (re-measured 2026-08-13 after the
  `@vitest/coverage-v8` 2→4 instrument change, which remaps the same suite lower): **92.15% lines,
  88.16% statements, 88.28% functions, 77.55% branches**; thresholds (91.5/87.5/87.5/77) sit just
  under it. **Ratchet only** — raise them as coverage rises, never lower them to make a red run
  go green (an instrument change is re-baselined with the evidence written into the config comment).
