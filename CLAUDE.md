...
- **MCP tokens are SITE-SCOPED** (migration 0028, `mcp_token.site_id`) — minted against the active site like a delivery key, which is the precedent (`verifyMcpToken` now returns `{userId, siteId}`, shaped like `verifyDeliveryKey`). The scope is a CREDENTIAL-level cap: it sets the server's default site and bounds every per-call `site`, so a scoped token asking for another site is refused, not quietly served. `NULL` = every site — what existing rows get (nothing in flight changes on deploy) and what Settings → MCP's "Every site" checkbox mints for a cross-site agent. The scope is fixed for the life of the process: HTTP mode REFUSES a bearer whose scope differs from the boot token's, because adopting it per request would be a shared write that concurrent requests race on.
