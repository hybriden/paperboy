...
/**
 * Does `provided` match the configured preview secret? Constant-time (S2-M11 —
 * the published package's compare, not a local copy), and the committed dev
 * default never matches in production (S2-M2), so a deploy that forgot to rotate
 * PREVIEW_SECRET silently exposes nothing rather than serving drafts to anyone
 * who knows the public default.
 */
