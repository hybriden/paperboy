...
describe("Start page (served at /)", () => {
  ...
  it("an editor can change the start page; /start then serves the new page", async () => {
    // New published page.
    const created = await s.app.inject({ method: "POST", url: "/api/v1/manage/content", headers: authHeaders(ed), payload: { type: "StandardPage", locale: "en", name: "Landing" } });
  ...
  it("an unpublished start page is NOT served publicly but IS under preview (no-leak)", async () => {
    const created = await s.app.inject({ method: "POST", url: "/api/v1/manage/content", headers: authHeaders(ed), payload: { type: "StandardPage", locale: "en", name: "DraftStart" } });
    const id = created.json().documentId;
